...
//#define SIM_SCGC6         System Clock Gating Control Register 6
//#define SIM_SCGC6_SPI0       SPI0 Clock Gate Control
...
public:
	...
	inline SPCRemulation & operator |= (int val) __attribute__((always_inline)) {
		...
		if (val & (1<<DORD)) SPI0_CTAR0 |= SPI_CTAR_LSBFE; // TODO: use bitband
		if (val & (1<<CPOL)) SPI0_CTAR0 |= SPI_CTAR_CPOL;
		if (val & (1<<CPHA)) SPI0_CTAR0 |= SPI_CTAR_CPHA;
		if ((val & 3) == 1) {
			// TODO: implement - is this ever really needed
		} else if ((val & 3) == 2) {
			// TODO: implement - is this ever really needed
		} else if ((val & 3) == 3) {
			SPI0_CTAR0 = (SPI0_CTAR0 & 0xFFFFFFF0) | SPI_CTAR_BR(6);
		}
	...
	inline SPCRemulation & operator &= (int val) __attribute__((always_inline)) {
		...
		if (!(val & (1<<CPHA))) SPI0_CTAR0 &= ~SPI_CTAR_CPHA;
		if ((val & 3) == 0) {
			SPI0_CTAR0 = (SPI0_CTAR0 & 0xFFFFFFF0) | SPI_CTAR_BR(1);
		} else if ((val & 3) == 1) {
			// TODO: implement - is this ever really needed
		} else if ((val & 3) == 2) {
			// TODO: implement - is this ever really needed
	...
	inline int operator & (int val) const __attribute__((always_inline)) {
		...
		if (val & (1<<SPE) && (!(SPI0_MCR & SPI_MCR_MDIS))) val |= (1<<SPE);
		if (val & (1<<MSTR) && (SPI0_MCR & SPI_MCR_MSTR)) val |= (1<<MSTR);
		return ret;
	}
	// TODO, is integer read operator needed?
private:
...
public:
	...
	inline SPSRemulation & operator |= (int val) __attribute__((always_inline)) {
		...
		//serial_phex(val);
		//serial_print("\n");
		if (val & (1<<SPI2X)) SPI0_CTAR0 |= SPI_CTAR_DBR;
		return *this;
	}
	inline SPSRemulation & operator &= (int val) __attribute__((always_inline)) {
		//serial_print("SPSR &= ");
